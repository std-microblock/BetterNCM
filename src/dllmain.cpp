...
std::string GetBacktrace(EXCEPTION_POINTERS* ExceptionInfo) {
	...
	while (StackWalk64(IMAGE_FILE_MACHINE_I386, GetCurrentProcess(), GetCurrentThread(), &stackFrame, contextRecord,
		...
		UnDecorateSymbolName(symbol->Name, name, 256, UNDNAME_COMPLETE);
...
std::wstring PrintExceptionInfo(EXCEPTION_POINTERS* ExceptionInfo) {
	...
	Stream << L"Exception code: 0x" << std::hex << ExceptionRecord->ExceptionCode << "(" << BNString(
		GetExceptionName(ExceptionInfo)) << ")" << std::endl;
...
#define IGNORE_ERROR_IGNORE(code) if(ExceptionInfo->ExceptionRecord->ExceptionCode==code){ return EXCEPTION_CONTINUE_EXECUTION;};
	...
	// IGNORE_ERROR_PASS_TO_NCM(0xe0000008); // Ignore UNKNOWN
	...
	int result = MessageBoxW(ncmWin,
		(L"很抱歉，网易云音乐崩溃了！\n\n" +
			...
			L"In Process: " + BNString(std::format("{:#x}", static_cast<int>(process_type))) +
			L"\n" +
