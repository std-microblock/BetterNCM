...
function main() {
    ...
    fetch("https://music.163.com/betterncm_api/getdir?").then(function (resp) { return resp.text() }).then(function (resp1) {
        addons = resp1.split(",");
        
        for (var x = 0; x < addons.length; x++) {
            var addon = addons[x];
            if(!addon.endsWith(".js"))continue;
            fetch("https://music.163.com/betterncm_api/read_file?" + addon)
